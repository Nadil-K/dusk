...
class RedisHitStore(HitStore):
    ...
    async def record(self, hit: HitEvent) -> None:
        ...
        pipe.xadd(
            self._stream_key(),
            {
                ...
                "enforced": "1" if hit.enforced else "0",
            },
            maxlen=100_000,
            approximate=True,
        )
    ...
    def _deserialize(self, entry: tuple) -> HitEvent:
        ...
        return HitEvent(
            ...
            enforced=fields.get("enforced") == "1",
        )

    async def recent_hits(self, query: HitQuery) -> list[HitEvent]:
        entries = await self._redis.xrevrange(self._stream_key(), count=query.limit)
